...
class CustomTableMenu {
  final String? tooltip;
  final List<BaseCustomTableMenuItem> items;
  final double? elevation;
  final BoxConstraints? constraints;
  final void Function(dynamic value)? onSelected;
  final ShapeBorder? shape;

  const CustomTableMenu(
      {required this.items,
      this.tooltip,
      this.elevation,
      this.constraints,
...
abstract class BaseCustomTableMenuItem {
  const BaseCustomTableMenuItem();

  Widget _build(BuildContext context);
}
class CustomTableMenuItem extends BaseCustomTableMenuItem {
  final VoidCallback? onTap;
  final Widget title;
  final Widget? subtitle;
  final Widget? leading, trailing;

  const CustomTableMenuItem(
      {this.onTap,
      required this.title,
      this.subtitle,
      this.leading,
...
class CustomTableMenuDivider extends BaseCustomTableMenuItem {
  final double height;

  const CustomTableMenuDivider({this.height = 0});

  @override
  Widget _build(BuildContext context) => Divider(height: height);
}

class FilterMenuItemBuilder extends BaseCustomTableMenuItem {
...
void showCustomMenu(
    {required BuildContext context, required List<BaseCustomTableMenuItem> items}) {
